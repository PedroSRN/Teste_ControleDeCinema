...
using Teste_ControleDeCinema.Aplicacao.ModuloAutenticacao;
...
namespace Teste_ControleDeCinema.Webapi.Controllers
{
    [Route("api/conta")]
    [ApiController]
    public class AutenticacaoController : ControleCinemaControllerBase
    {
        ...
        private TokenViewModel GerarJwt(Usuario usuario)
        {
            ...
            var key = Encoding.ASCII.GetBytes("SegredoDoControleDeCinema");
            DateTime dataExpiracao = DateTime.UtcNow.AddHours(8);
            var token = tokenHandler.CreateToken(new SecurityTokenDescriptor
            {
                Issuer = "ControleCinema",
